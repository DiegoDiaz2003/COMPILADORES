...
TokenType identifyToken(const string& word) {
    ...
    for (const string& keyword : keywords) { // Recorre la lista de palabras clave.
        if (word == keyword) return TokenType::KEYWORD; // Si la palabra es una palabra clave, retorna KEYWORD.
    }

    // Verificar si es un número (simplificación: solo enteros)
    if (all_of(word.begin(), word.end(), ::isdigit)) return TokenType::NUMBER; // Si todos los caracteres son dígitos, retorna NUMBER.

    // Verificar si es un símbolo
    if (word.size() == 1 && string("+-*/").find(word) != string::npos) return TokenType::SYMBOL; // Si es un símbolo (+-*/), retorna SYMBOL.
...
int main() {
    ...
    for (const Token& token : tokens) { // Recorre el vector de tokens.
        cout << "Token: " << token.value << ", Tipo: "; // Imprime el valor del token.
        switch (token.type) { // Dependiendo del tipo del token...
            case TokenType::KEYWORD: cout << "Palabra clave"; break; // Imprime "Palabra clave" si es una palabra clave.
            case TokenType::IDENTIFIER: cout << "Identificador"; break; // Imprime "Identificador" si es un identificador.
            case TokenType::NUMBER: cout << "Número"; break; // Imprime "Número" si es un número.
            case TokenType::SYMBOL: cout << "Símbolo"; break; // Imprime "Símbolo" si es un símbolo.
            default: cout << "Desconocido"; break; // Imprime "Desconocido" si no se reconoce el tipo (no debería suceder).
        }
